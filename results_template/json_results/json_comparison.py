...
def compare_test_results(json_dicts, file_names):
    ...
    for test_name, configs in results_by_test.items():
        for idx, results in sorted(configs.items()):
            if len(set(results)) > 1:  # mismatch detected
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python compare_tests.py file1.json file2.json ...")
        sys.exit(1)

    file_paths = sys.argv[1:]
    json_dicts = load_json_files(file_paths)
    compare_test_results(json_dicts, file_paths)
